...
# Fix all relative imports that might be problematic
echo "🔍 Checking for problematic relative imports..."

# 1. Fix all "../src/" imports in the app directory to use absolute imports
find ./app -name "*.js" -exec sed -i '' -e 's|from "\.\./src/|from "@/|g' {} \;

# 2. Fix all "../../../src/" imports to use absolute imports
find ./app -name "*.js" -exec sed -i '' -e 's|from "\.\./\.\./\.\./src/|from "@/|g' {} \;

# 3. Fix barber layout file imports specifically
if [ -f "./app/(app)/(barber)/_layout.js" ]; then
  sed -i '' 's|from "../../../src/styles/theme"|from "@/styles/theme"|g' ./app/(app)/(barber)/_layout.js
  echo "✅ Fixed barber layout theme import"
fi

# 4. Fix customer layout file imports if it exists
if [ -f "./app/(app)/(customer)/_layout.js" ]; then
  sed -i '' 's|from "../../../src/styles/theme"|from "@/styles/theme"|g' ./app/(app)/(customer)/_layout.js
...
rm -f app.json
echo "✅ Removed app.json if it existed"
